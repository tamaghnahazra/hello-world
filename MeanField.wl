...
TMDHamNonInt = Function[{kx, ky},
  Module[{k={kx, ky}, T1, T1c, T2, \[Lambda]},
    ...
    \[Lambda][k_,Up,Dn]:=-I \[Alpha] Sum[((a.{0,1})*\[Sigma][1][[Up,Dn]]-(a.{1,0})*\[Sigma][2][[Up,Dn]])Exp[I k.a],{a,{a1,a2,a3}}];
    \[Lambda][k_,Dn,Up]:=-I \[Alpha] Sum[((a.{0,1})*\[Sigma][1][[Dn,Up]]-(a.{1,0})*\[Sigma][2][[Dn,Up]])Exp[I k.a],{a,{a1,a2,a3}}];
    {{-\[Mu]+mz-T2    ,        T1,          0,         \[Lambda][k,Up,Dn]},
     {      T1c   , -\[Mu]-mz+T2,-\[Lambda][-k,Up,Dn],         0},
     {        0   ,\[Lambda][k,Dn,Up],   -\[Mu]+mz+T2,        T1},
     {-\[Lambda][-k,Dn,Up],         0,        T1c, -\[Mu]-mz-T2}}
  ]
];

TMDHamMF = Function[{\[CapitalGamma]\[Mu],\[CapitalGamma]mz,\[CapitalDelta]},
  Function[{kx, ky},
    Module[{hKinetic1, hKinetic2, hShift, hPairing},
      hKinetic1 = TMDHamNonInt[ kx,  ky] - U/2 IdentityMatrix[numSpin * numOrbital] + \[CapitalGamma]\[Mu] * IdentityMatrix[numSpin * numOrbital] - \[CapitalGamma]mz * DiagonalMatrix[{1,-1,1,-1}];
      hKinetic2 = TMDHamNonInt[-kx, -ky] - U/2 IdentityMatrix[numSpin * numOrbital] + \[CapitalGamma]\[Mu] * IdentityMatrix[numSpin * numOrbital] - \[CapitalGamma]mz * DiagonalMatrix[{1,-1,1,-1}];
...
TMDHamMF2 = Function[{\[CapitalGamma]\[Mu], \[CapitalGamma]mz, \[CapitalDelta]os, \[CapitalDelta]nn},
  Function[{kx, ky},
    Module[{k={kx,ky}, hKinetic1, hKinetic2, hShift, hPairing, \[CapitalDelta]AB, \[CapitalDelta]BA, \[CapitalDelta]0},
      hKinetic1 = TMDHamNonInt[ kx,  ky] - ((U+6V)/2) * IdentityMatrix[numSpin * numOrbital] + \[CapitalGamma]\[Mu] * IdentityMatrix[numSpin * numOrbital] - \[CapitalGamma]mz * DiagonalMatrix[{1,-1,1,-1}];;
      hKinetic2 = TMDHamNonInt[-kx, -ky] - ((U+6V)/2) * IdentityMatrix[numSpin * numOrbital] + \[CapitalGamma]\[Mu] * IdentityMatrix[numSpin * numOrbital] - \[CapitalGamma]mz * DiagonalMatrix[{1,-1,1,-1}];;
...
ComputeMF2 = Function[{kx, ky, eigenvalues, eigenvectors},
  Module[{k={kx,ky}, \[Psi], e, f, u, v, \[Rho], t, \[CapitalGamma]\[Mu]part, \[CapitalGamma]mzpart, \[CapitalDelta]part, \[CapitalDelta]nnpart},
    (*
    \[Psi] = ArrayReshape[eigenvectors, {numNambu,  numSpin*numOrbital,  numNambu,  numSpin,  numOrbital}];
    e = ArrayReshape[eigenvalues,  {numNambu,  numSpin*numOrbital}];
    u = \[Psi][[1, All, 1, All, All]];
    ...
    *)
    ...
    \[CapitalGamma]\[Mu]part = ((U + 6 V)/4) * (\[Rho][Up,A,Up,A,{0,0}] + \[Rho][Up,B,Up,B,{0,0}] + \[Rho][Dn,A,Dn,A,{0,0}] + \[Rho][Dn,B,Dn,B,{0,0}]);
    \[CapitalGamma]mzpart = ((U + 6 V)/4) * (\[Rho][Up,A,Up,A,{0,0}] - \[Rho][Up,B,Up,B,{0,0}] + \[Rho][Dn,A,Dn,A,{0,0}] - \[Rho][Dn,B,Dn,B,{0,0}]); (* Note: U can be replaced by a function of (kx, ky) *)
    ...
    {\[CapitalGamma]\[Mu]part, \[CapitalGamma]mzpart, \[CapitalDelta]part, \[CapitalDelta]nnpart}
  ]
];
...
(* Given \[CapitalGamma], \[CapitalDelta],
   ...
   compute contributionos of new \[CapitalGamma] and new \[CapitalDelta] at each (kx, ky),
   and return their means (over BZ) *)
CollectMF2 = Function[{\[CapitalGamma]\[Mu], \[CapitalGamma]mz,  \[CapitalDelta]os, \[CapitalDelta]nn},
  Module[{hamMF= TMDHamMF2[\[CapitalGamma]\[Mu], \[CapitalGamma]mz, \[CapitalDelta]os, \[CapitalDelta]nn]},
    1/(N1*N2) Sum[
